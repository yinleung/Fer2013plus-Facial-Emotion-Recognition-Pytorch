...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.attention_layers = nn.ModuleList([
            SELayer(channel=64).to(device),
            SELayer(channel=128).to(device),
            SELayer(channel=256).to(device),
            SELayer(channel=512).to(device),
        ])
    ...
    def forward(self, x):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.attention_layers[0](out)
        out = self.layer2(out)
        out = self.attention_layers[1](out)
        out = self.layer3(out)
        out = self.attention_layers[2](out)
        out = self.layer4(out)
        out = self.attention_layers[3](out)
